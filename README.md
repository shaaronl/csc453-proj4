...
Block 0 is always the superblock, which stores the total block count and the first free block.
...
Inode blocks store an 8-character alphanumeric file name, file size, first data block, and read-only flag. File data blocks store 252 bytes of content beginning at byte 4.
...
- Timestamp storage uses the platform's native `time_t` representation so it's not very portable
